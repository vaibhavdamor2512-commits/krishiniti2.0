...
const client = axios.create({ baseURL: import.meta.env.VITE_API_URL || 'http://localhost:8000/api', timeout: 8000 })
...
const fallback = async (work, value) => {
  try { return (await work()).data }
    ...
    throw new Error(error.response?.data?.detail || 'Please try again.')
  }
}

export const api = {
  login: (mobile,password) => fallback(()=>client.post('/auth/login',{mobile,password}), {access_token:'offline-demo',user:demoUser}),
  register: (payload) => fallback(()=>client.post('/auth/register',payload), {access_token:'offline-demo',user:{...demoUser,...payload}}),
  me: () => fallback(()=>client.get('/auth/me'), demoUser),
  language: (preferred_language) => fallback(()=>client.put('/auth/language',{preferred_language}), {...demoUser,preferred_language}),
  ...
  createField: (payload) => fallback(()=>client.post('/fields',payload), {...demoField,...payload,id:Date.now(),area:2.4}),
  updateField: (id,payload) => fallback(()=>client.put(`/fields/${id}`,payload), {...demoField,...payload,id}),
  recommendations: (field_id) => fallback(()=>client.post('/crops/recommend',{field_id,season:'Kharif'}), {field_id,area_acres:2.4,weather:demoWeather,recommendations:demoRecommendations,data_mode:'demo'}),
  ...
  readNotification: (id) => fallback(()=>client.put(`/notifications/${id}/read`), {id,is_read:true}),
}
