...
export const PageHeader = ({eyebrow,title,description,action,back}) => <div className="page-header"><div>{back&&<Link className="back-link" to={back}><ArrowLeft size={16}/> Back</Link>}{eyebrow&&<p className="eyebrow">{eyebrow}</p>}<h1>{title}</h1>{description&&<p>{description}</p>}</div>{action}</div>
...
export const Loading = () => <div className="loading"><LoaderCircle className="spin"/><span>Gathering field information…</span></div>
