...
export function FarmDetail(){
  ...
  return <><PageHeader back="/farms" eyebrow={t('farmDetails')} title={farm.name} description={`${farm.location} · ${farm.total_area} ${t('acres')}`} action={<Link className="button button-primary" to={`/fields/new?farm=${id}`}><Plus size={17}/> {t('drawField')}</Link>}/><div className="panel map-overview"><div className="field-shape large"/><div><p className="eyebrow">{t('location')}</p><h2>{farm.location}</h2><p>{t('farmCentre')}: {farm.latitude}, {farm.longitude}</p><small>{t('mapBoundaryNote')}</small></div></div><h2 className="section-title">{t('fields')}</h2><div className="field-list">{fields.map(field=><Link to={`/fields/${field.id}`} key={field.id} className="field-row"><div className="crop-icon"><Sprout/></div><div><h3>{field.name}</h3><p>{field.current_crop||t('noCropSelected')} · {field.area} {t('acres')}</p></div><span>{t('soilMoisture')} {field.soil?.moisture}%</span><ArrowRight/></Link>)}</div></>
}
