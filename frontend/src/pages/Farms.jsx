...
export function FarmDetail(){
  const {t}=useApp();const {id}=useParams();const [farm,setFarm]=useState(null);const [fields,setFields]=useState(null);const [error,setError]=useState('')
  useEffect(()=>{Promise.all([api.farm(id),api.fields(id)]).then(([a,b])=>{setFarm(a);setFields(b)}).catch(err=>setError(err.message||t('unableLoad')))},[id,t])
  if(error)return <div className="form-error" role="alert">{error}</div>;if(!farm||!fields)return <Loading/>
  return <><PageHeader back="/farms" eyebrow={t('farmDetails')} title={farm.name} description={`${farm.location} · ${farm.total_area} ${t('acres')}`} action={<Link className="button button-primary" to={`/fields/new?farm=${id}`}><Plus size={17}/> {t('drawField')}</Link>}/><div className="panel map-overview"><div className="field-shape large"/><div><p className="eyebrow">{t('location')}</p><h2>{farm.location}</h2><p>{t('farmCentre')}: {farm.latitude}, {farm.longitude}</p><small>{t('mapBoundaryNote')}</small></div><iframe src={`https://www.google.com/maps?q=${farm.latitude},${farm.longitude}&z=15&output=embed`} width="100%" height="300" style={{border:0,marginTop:'20px',borderRadius:'8px'}} allowFullScreen loading="lazy"></iframe></div><h2 className="section-title">{t('fields')}</h2><div className="field-list">{fields.map(field=><Link to={`/fields/${field.id}`} key={field.id} className="field-row"><div className="crop-icon"><Sprout/></div><div><h3>{field.name}</h3><p>{field.current_crop||t('noCropSelected')} · {field.area} {t('acres')}</p></div><span>{t('soilMoisture')} {field.soil?.moisture}%</span><ArrowRight/></Link>)}</div></>
}
